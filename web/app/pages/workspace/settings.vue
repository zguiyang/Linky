    ...
    <div class="flex-1 flex flex-col sm:flex-row overflow-hidden">
      ...
      <aside class="w-full sm:w-64 border-r border-(--border-subtle) bg-(--bg-surface) p-2 md:p-4 flex flex-row sm:flex-col gap-1 overflow-x-auto sm:overflow-visible">
        <u-button
          ...
          variant="ghost"
          color="neutral"
          class="justify-start gap-3 px-3 py-2.5 flex-shrink-0"
          ...
          >
            <section>
              ...
              <div class="space-y-8">
                ...
                <div class="pt-4 border-t border-(--border-subtle) flex flex-col sm:flex-row justify-end items-center gap-3 sm:gap-4">
                  <span
                    v-if="lastSaved"
                    class="text-xs text-(--text-muted) order-2 sm:order-1"
                  >
                    最后保存于: {{ lastSaved }}
                  </span>
                  <u-button
                    color="primary"
                    size="lg"
                    class="px-8 min-h-11 order-1 sm:order-2 w-full sm:w-auto"
                    :loading="saving"
                    @click="saveSettings"
                  >
                    保存更改
