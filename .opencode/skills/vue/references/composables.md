...
This project uses `eslint-plugin-vue-composable` to enforce composable best practices automatically.
...
export default {
  ...
  rules: {
    // Composables must be called at top level of <script setup>
    'vue-composable/composable-placement': 'error',

    // Lifecycle hooks must be at top level
...
|------|-------------|----------|
| `vue-composable/composable-placement` | Composables must be called at top level of `<script setup>` | error |
| `vue-composable/lifecycle-placement` | Lifecycle hooks must be at top level | error |
| `vue/define-macros-order` | Macro order must be `defineOptions` → `defineProps` → `defineEmits` → `defineSlots` | error |
| `vue/define-props-destructuring` | Prefer destructuring `defineProps` | warning |

### Common Violations

**Composable in event handler**:
```typescript
// ❌ Wrong
const handleClick = () => {
...
// ✅ Correct
onMounted(() => {
  fetchData()  // Call inside hook
})
```

**Macro order violation**:
```typescript
// ❌ Wrong
const emit = defineEmits(['update'])  // ❌ Should be after defineProps
